...
# Big cycle
def scan_1scw_for_flares(fn, par=par_default, flares=None, verbose=True, check_bad_pixels=False, pif_fn=None, isgri_fstr=None, use_roll_avg=False, mod_good=None):
	...
	if isgri_fstr is not None:
		...
		if n_gti > 0:
			if use_roll_avg:
				r_ = r.copy()
				mu_impute, std_impute = sigma_clipping(r[gti])
				# better impute poisson(mu) than just mu, to keep higher std
				r_[~gti] = poisson(mu_impute, size=len(r_[~gti]))
				mu, std = sigma_clipping_roll(r_,window=2**(par['n_binnings']+2))
				del r_
			else:
				r_ = r.copy()
				mu_impute, std_impute = sigma_clipping(r[gti])
				r_[~gti] = poisson(mu_impute, size=len(r_[~gti]))
				mu, std = sigma_clipping(r_)
				del r_
				# mu, std = sigma_clipping(r[gti])
		#	r_[~gti] = mu
		else:
			if use_roll_avg:
				mu, std = sigma_clipping_roll(r, window=2**(par['n_binnings']+2))
			else:
...
def scan_scws_for_flares(file_list, par=par_default, flares=None, patological_scw=[], verbose=True, isgri_fstr=None, use_roll_avg=False):

	if flares is None:
		flares = pd.DataFrame(columns=column_names)

	for fn in file_list:
		flares = scan_1scw_for_flares(fn, par=par, flares=flares, verbose=verbose, isgri_fstr=isgri_fstr, use_roll_avg=use_roll_avg)
...
if __name__ == "__main__":
	try:
		opts, args = getopt.getopt(sys.argv[1:], "ho:vd:p:s:wP:N:ZIR",\
		 ["help", "output=","verbose","dir=","par=","sigma-threshold=","walltime",\
		 "poisson-threshold=","nsigma=", "no-zero-mean","isgri-eventlist=", "rolling-average"])
	...
	flares = scan_scws_for_flares(filelist, par=par, patological_scw=patological_scw, verbose=verbose, isgri_fstr=isgri_fstr, use_roll_avg=use_roll_avg)
	if walltime:
		t1 = time.time()
