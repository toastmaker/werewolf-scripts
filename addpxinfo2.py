...
def file_is_empty(path):
    ...
    file_empty = os.stat(path).st_size==0 if file_exists else False   
    return file_empty

"""
Usage:
addpxinfo.py flares.csv eventlist_template mask_template low_e high_e pif_th
"""
...
if len(sys.argv) < 7:
	print("Usage:\naddpxinfo.py flares.csv ""/storage/topinka/SGR1806/out/{rev}/{scw}_evts_h.fits.gz"" ""/storage/topinka/SGR1806/masks/{rev}/{scw}_isgri_model.fits.gz"" low_e high_e pif_th")
	sys.exit(1)
	
flares_fn = sys.argv[1]
fn_template=sys.argv[2]
fn_mask_template=sys.argv[3]
...
for i in range(len(flares)):
	...
	fn = fn_template.format(rev=scw[0:4], scw=scw)
	fn_mask = fn_mask_template.format(rev=scw[0:4], scw=scw)
...
if len(flares) > 0:
    ...
    with open(fn_flares_new, 'a') as the_file:
