...
for year in `seq 2003 2022`; do ${CMD} ${RA} ${DEC} "${year}-01-01 00:00:00" "${year}-06-30 23:59:59" | grep '10$'; ${CMD} ${RA} ${DEC} "${year}-07-01 00:00:00" "${year}-12-31 23:59:59" | grep '10$'; done > ${SRC}.scw
...
cat ${SRC}_scw_checked.txt | grep -v ^# | grep -v '\-999.000' | awk '(NR>0) { if ( ( $8*$8< 15*15) && ($9*$9 <15*15) ) { print substr($1,1,12) "0010" }}' | sort | uniq > ${SRC}_fov.scw
