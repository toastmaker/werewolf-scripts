...
def findpeaks_Li(data_, mu, std, Nvar=5, peak_th_ns=4.):
	"""
	keep only peaks that are larger than peak_th_ns*std above mu
	
	based on Li & Fenimore
	"""
	data = data_.copy()-mu
	N = len(data)
	peakList = []
	for i in range(N):
		...
		i1 = i-1
		foundC1 = False
		while (i1 >= 0 and foundC1 == False):
			C1 = data[i1]
			if (C1 > Cp): break
			if ( (Cp - C1) > Nvar*sqrtCp): # np.sqrt(Cp)):
				foundC1 = True
			i1 = i1 -1
		if (foundC1 == False): continue

		# go to the right
		i2 = i+1
		foundC2 = False
		while (i2 < N and foundC2 == False):
			C2 = data[i2]
			if (C2 > Cp): break
			if ( (Cp - C2) > Nvar*sqrtCp): # np.sqrt(Cp)):
				foundC2 = True
			i2 = i2 +1
		if (foundC2 == False): continue

		if (foundC1==True and foundC2 == True):
			if data[i] > peak_th_ns*std:
				peakList.append(i)
	return np.array(peakList)

def split_flares(lc_, mu, std, peaks, th_ns ,m, fls=None):
	"""
	It splits light curves into flares identified by peaks into individual flares
	only if the light curve drops below mu + th_ns*std for more than m successive bins.
	
	It returns an array of individual flares found. The other flares than the
	actual flare of the original light curve are removed (padded with mu).
	
	Ps: It uses recursion, so don't go too deep.
	"""
	if fls is None: fls = []
	lc = lc_.copy()
	N = len(lc)
	c = 0
	if len(peaks) <= 1:
		fls.append(lc) # nothing to split
		return np.array(fls).copy()

	for j in range(peaks[0], N):
		if j > peaks[1]:
			peaks = peaks[1:]
			if len(peaks) < 1:
				fls.append(lc) # nothing to split
		...
		if len(peaks) == 1:
			fls.append(lc) # nothing to split
			return np.array(fls).copy()
		if lc[j] < th_ns*std + mu:
			c += 1
		else:
			c = 0
		if c >= m:
			fl = mu*np.ones_like(lc)
			fl[ :j ] = lc[ :j ]
			fls.append(fl)
			newlc = mu*np.ones_like(lc)
			newlc[ j:] = lc[ j:]
			split_flares(newlc, mu, std, peaks[1:], th_ns, m, fls=fls)
			break
	return np.array(fls).copy()
...
def rollmean(x, w):
#	cval = np.mean(x)
	mu = uniform_filter1d(x, size=w, mode='reflect') #, cval=cval)
	return mu
		
def rollstd(x, w):
	mu = rollmean(x, w)
	return np.sqrt( np.sum( (mu-x)**2) /w)
...
def sigma_clipping_roll(r, niter=3, ns=3, window=100, mode="gaussian"):
	x_ = r.copy() 
	for i in range(niter):
		if mode=="gaussian":
			mu, std = rollmean(x_,w=window), rollstd(x_, w=window)
			mu0 = np.mean(x_)
			# if ns*std < 1:
			# 	# if there is so few counts that it would clip everything, forget about clipping
			#	return mu, std 
			x_ = np.where( np.abs(x_ - mu0) <= ns*std, x_, mu)
		else:
			mu = rollmean(x_, w=window)
			std = np.sqrt(mu)
			mu0 = np.mean(x_)
			th = poiss_ns_threshold(mu0, ns)
			x_ = np.where(x_ <= th, x_, mu) 
	if mode=="gaussian":	
		mu, std = rollmean(x_, w=window), rollstd(x_, w=window)
	else:
		mu = rollmean(x_, w=window)
		std = np.sqrt(mu)
	if np.sum(mu)/len(mu) < 1e-7:
		# forget about clipping when there is so few counts
		mu, std = rollmean(r, w=window), rollstd(r, w=window)
	return mu, std
...
def get_lightcurve(eventlist_fn, par=par_default, time0 = None, \
	...
	with fits.open(eventlist_fn) as hdu:
		...
		if antipif==True:
			antipif_th = 1.-par['pif_threshold']
			filtr = (energy >= par['low_e']) & (energy <= par['high_e']) \
		        & (pif <= antipif_th) & select_flag
		else:
			filtr = (energy >= par['low_e']) & (energy <= par['high_e']) \
		...
		tstart = 0 # arr_times[0] # hope they are sorted already, it's faster than arr_times.min()
