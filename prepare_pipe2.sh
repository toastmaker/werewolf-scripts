...
echo "FYI, this is the faster version... does not check the quality of the scw's"
...
for year in `seq 2003 2023`; do ${CMD} ${RA} ${DEC} "${year}-01-01 00:00:00" "${year}-06-30 23:59:59" | grep '10$'; ${CMD} ${RA} ${DEC} "${year}-07-01 00:00:00" "${year}-12-31 23:59:59" | grep '10$'; done > ${SRC}.scw

wc -l ${SRC}.scw

echo "Check scws..."
${HOME}/scripts/idl_checkscw2.sh ${SRC} ${RA} ${DEC} &> ${HOME}/${SRC}/lists/idl_checkscw2_${SRC}.out

echo "Select scws and in fov..."
#  columns 6 7 

cat ${SRC}_scw_checked2.txt | grep -v ^# | grep -v '\-999.000' | awk '(NR>0) { if ( ( $6*$6< 15*15) && ($7*$7 <15*15) ) { print substr($1,1,12) }}' | sort | uniq > ${SRC}_fov.scw
awk '{print substr($1,1,4)}' ${SRC}_fov.scw | sort | uniq > ${SRC}_fov.rev
wc -l ${SRC}_fov.scw
wc -l ${SRC}_fov.rev
